...
export class ClientManager{
    ...
    static setClient(type: typeClient, client: AuthClient): boolean{
        let c = ClientManager;
        try{
            if(type == "queue"){
                c.queue.set(client.connection.sessionId, client);
            }
            if(type == "client_ip"){
                c.client_ByIP.set(client.connection.socket.remoteAddress, client);
            }
            return true;
        } catch (err){
            return false;
