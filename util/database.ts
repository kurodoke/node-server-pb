...
class Database {
    ...
    constructor() {
        this._connection = new Sequelize({
            ...
            password: DatabaseSetting.dbPass
                ? DatabaseSetting.dbPass.toString()
                : "123456",
            port: DatabaseSetting.dbPort ? DatabaseSetting.dbPort : 5432,
            database: DatabaseSetting.dbName
                ? DatabaseSetting.dbName
                : "postgres",
            username: DatabaseSetting.dbUser
                ? DatabaseSetting.dbUser
                : "postgres",
            logging: AuthSettingServer.debugMode
                ? (msg) => Log.getLogger("database").info(msg)
                : false,
        });
        ...
        this.model.player_inventory.belongsTo(this.model.player, {
            foreignKey: "playerId",
        });
        this.model.player_inventory.belongsTo(this.model.item, {
            foreignKey: "itemId",
        });
        ...
        this.model.coupon.belongsTo(this.model.player, {
            foreignKey: "playerId",
        });
        ...
        this.model.message.belongsTo(this.model.player, {
            foreignKey: "senderId",
            as: "Sender",
        });
        ...
        this.model.message.belongsTo(this.model.clan, {
            foreignKey: "clanId",
            as: "Clan",
        });
    ...
    async getData(query: string) {
        let data = await this._connection.query(query, {
